use {
   ...
   godot::{
      classes::{
         AtlasTexture, FileAccess, IEditorImportPlugin, ResourceLoader, ResourceSaver, SpriteFrames,
         Texture2D,
      },
      global::Error,
      prelude::*,
...
impl IEditorImportPlugin for GdxAtlasImporter {
   ...
   fn import(
      ...
      for line in srcAtlasContents.lines() {
         ...
         if &line[..1] == "\t" {
            ...
            match key {
               ...
               "offsets" => {
                  c.margin = Rect2::from_components(
                     x,
                     h - y - c.region.size.y,
                     w - c.region.size.x,
                     h - c.region.size.y,
      ...
      assert_eq!(
         c.resourceSaver.save_ex(c.spriteFrames).path(spriteFramesTresPath.clone()).done(),
         Error::OK
      );
